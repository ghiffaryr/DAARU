    ...
    for i in range(epoch):
        ...
        if train_mode == 'dann':
            ...
            if ((i + 1) % 100 == 0):
                ...
                validation_y_pred = source_classification_model.predict(validation_x)
                val_loss = dice_loss(validation_y,validation_y_pred)
                val_iou = iou(validation_y,validation_y_pred)
                val_iou_thresholded = iou_thresholded(validation_y,validation_y_pred)
                cost_val_class = [val_loss, val_iou, val_iou_thresholded]
                val_class_history.append(cost_val_class)   
                   
                X1, y1 = X1.astype('float32'), y1.astype('float32')
                test_y_pred = source_classification_model.predict(X1)
                test_loss = dice_loss(y1,test_y_pred)
                test_iou = iou(y1,test_y_pred)
                test_iou_thresholded = iou_thresholded(y1,test_y_pred)
                cost_test_class = [test_loss, test_iou, test_iou_thresholded]
                test_class_history.append(cost_test_class)

                print('{:d}s - domain_loss: {:.4f} - domain_acc: {:.4f} - class_loss: {:.4f} - class_iou: {:.4f} - class_iou_thresholded: {:.4f} - class_val_loss: {:.4f} - class_val_iou: {:.4f} - class_val_iou_thresholded: {:.4f} - class_test_loss: {:.4f} - class_test_iou: {:.4f} - class_test_iou_thresholded: {:.4f}\n'.format(end_time-start_time, cost_domain[0], cost_domain[1], cost_class[0], cost_class[1], cost_class[2], val_loss, val_iou, val_iou_thresholded, test_loss, test_iou, test_iou_thresholded))
                
                if cost_test_class[2]>cost_test_class_prev:
                    ...
                    print('Epoch {:05d}: test_iou_thresholded improved from {:.4f} to {:.4f}, saving weight and model to {:s}'.format(iterate, cost_test_class_prev, cost_test_class[2], save_dir))
                    cost_test_class_prev = cost_test_class[2]
            ...
            if ((i + 1) % epoch == 0):
                ...
                return domain_history,class_history,val_class_history,test_class_history
            j+=1
